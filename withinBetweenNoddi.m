...
for l=3:length(subjects)	
   ...
   % l=3
    ...
    cd(currD)
    fa_path = dir('*FA_matrixts.csv')
    fa_mat = csvread(sprintf(fa_path.name),1,0);
    %figure, imagesc(fa_mat); colormap(jet); set(gcf,'color','white') 

    cd(currD)
    odi_path = dir('*ODI_matrixts.csv'); 
    sc_mat = csvread(sprintf(odi_path.name),1,0); 
    figure, imagesc(log(sc_mat)); colormap(jet); set(gcf,'color','white')
